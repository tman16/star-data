...
function checkOffStarMap({ pt, text_metrics }) {
  ...
  Celestial.display = function (config) {
    ...
    var ratio = projectionSetting.ratio,
      ...
      //console.log('projectionSetting.scale',projectionSetting.scale)
    ...
    function load() {
      ...
      var mw = getData(mw_data, cfg.transform);
      ...
      var con = getData(constellations_data, cfg.transform);
      ...
      var cb = getData(constellations_borders_data, cfg.transform);
      ...
      var conl = getData(constellations_lines_data, cfg.transform);
      ...
      var st = getData(cfg.stars.data === 'stars.6.json' ? star6_data : star8_data, cfg.transform);
      ...
      //if (error) return console.warn(error);
      Object.assign(starnames, starnames_data);
      redraw();
      //});
      ...
      var pl = getPlanets(planets_data, cfg.transform);
    ...
    function redraw() {
      ...
      let text_metrics = context.measureText('N');
      context.fillText('N', (getWidth() / 2) - (text_metrics.width / 2), 0);
      text_metrics = context.measureText('W');
      context.fillText('W', getWidth(), (getWidth() / 2) - text_metrics.actualBoundingBoxAscent);
      text_metrics = context.measureText('S');
      context.fillText('S', (getWidth() / 2) - (text_metrics.width / 2), getWidth() - text_metrics.actualBoundingBoxDescent - text_metrics.actualBoundingBoxAscent);
      text_metrics = context.measureText('E');
      context.fillText('E', 0 + text_metrics.width, (getWidth() / 2) - text_metrics.actualBoundingBoxAscent);
      ...
      if (cfg.constellations.names) {
        ...
        let constellation_names_rects = [];
        container.selectAll(parentElement + " .constname").each(function (d) {
          if (clip(d.geometry.coordinates, 0.9)) {
            setStyleA(d.properties.rank, cfg.constellations.nameStyle);
            var pt = mapProjection(d.geometry.coordinates);
            //Check if text off the page X axis
            let text_metrics = context.measureText(constName(d));
            let collision = checkTextCollision({ pt, text_metrics, constellation_names_rects })
            context.fillText(constName(d), collision[0], collision[1]);
            let tl = [collision[0] - (text_metrics.width / 2), collision[1] - text_metrics.actualBoundingBoxAscent];
            let br = [collision[0] + (text_metrics.width / 2), collision[1] + text_metrics.actualBoundingBoxDescent];
            constellation_names_rects.push({ tl: tl, br: br })
          }
        });
      }
      ...
      if (cfg.stars.show) {
        ...
        container.selectAll(parentElement + " .star").each(function (d) {
          if (clip(d.geometry.coordinates) && d.properties.mag <= cfg.stars.limit) {
            ...
            //Add shadow
            context.save();
            context.shadowColor = cfg.background.fill;
            context.shadowBlur = r * 0.2;
            context.fillStyle = starColor(d);
            context.beginPath();
            context.arc(pt[0], pt[1], r * 0.9, 0, 2 * Math.PI);
            context.closePath();
            context.fill();
            context.restore();
        ...
        container.selectAll(parentElement + " .planet").each(function (d) {
          ...
          if (clip(pos)) {
            ...
            if (cfg.planets.symbolType === "letter") {
              ...
              context.fillStyle = sym.fill;
              let check_planet = getPlanetSVG({ code: id });
              if (check_planet) {
                Canvas.symbol().type("crescent").size(r * r).planetSVG(check_planet).age(p.ephemeris.age).position(pt)(context);

              } else {
                Canvas.symbol().type("crescent").size(r * r).age(p.ephemeris.age).position(pt)(context);
              }
            } else if (cfg.planets.symbolType === "disk") {
              //console.log('CHECKFORSUN', id)
              //console.log('p.ephemeris', p.ephemeris)
              r = has(sym, "size") && isNumber(sym.size) ? sym.size * adapt : planetSize(p.ephemeris);
              let check_planet = getPlanetSVG({ code: id });
              //console.log('check_planet', check_planet)
              if (check_planet) {
                Canvas.symbol().type('planet').size(r * 3).planetSVG(check_planet).position(pt)(context);
              } else {
                //console.log('sym.fill', sym.fill)
                context.fillStyle = sym.fill;
            ...
            if (cfg.planets.names) {
              ...
              //Check if text off the page X axis
              let text_metrics = context.measureText(name);
              let collision = checkTextCollision({ pt, text_metrics, constellation_names_rects: planet_names_rects });
              context.fillText(name, collision[0], collision[1]);
              //context.fillText(name, pt[0], pt[1]);
              //console.log('PLANET CALLED ' + name + ' at ', pt[0], pt[1], ' and updated to ', collision)
              let tl = [collision[0] - (text_metrics.width / 2), collision[1] - text_metrics.actualBoundingBoxAscent];
              let br = [collision[0] + (text_metrics.width / 2), collision[1] + text_metrics.actualBoundingBoxDescent];
              planet_names_rects.push({ tl: tl, br: br });
            }
          }
        });
      }
    ...
    function clip(coords, adj = 1) {
      return projectionSetting.clip && d3.geo.distance(cfg.center, coords) > halfπ * adj ? 0 : 1;
    }
  ...
  if (typeof module === "object" && module.exports) {
    var d3js = require('./lib/d3.js'),
      ...
      mw_data = require('./mw.json'),
      constellations_data = require('./constellations.json'),
      constellations_borders_data = require('./constellations.borders.json'),
      constellations_lines_data = require('./constellations.lines.json'),
  ...
  */
  ...
  Canvas.symbol = function () {
    ...
    var draw_symbol = {
      "planet": async function (ctx) {
        //console.log('planetSVG', planetSVG())
        //document.createElement('svg')
        var s = size(),i;
        if (planet_objects) {
          i = planet_objects.find(x=>x.url===planetSVG()).img;
          //console.log('SPECIAL NEW THING',i)
        } else {
        i = new Image();
        i.crossOrigin = 'anonymous';
        i.src = planetSVG();
        //console.log('LOADINGPLANET BEFORE',i);
        await i.decode();
        //console.log('LOADINGPLANET AFTER',i);
        }
        ctx.drawImage(i, pos[0] - (s / 2), pos[1] - (s / 2), s, s);
        /*i.onload = function(){
          ctx.drawImage(i, pos[0], pos[1]);
        }*/
        return s;
      },
      "circle": function (ctx) {
      ...
      "crescent": function (ctx) {
        ...
        if (!!planetSVG()) {
          ...
          //Draw clip
          ctx.save();
          ctx.fillStyle = cfg.background.fill;
          ctx.beginPath();
          ctx.moveTo(pos[0], pos[1]);
    ...
    canvas_symbol.planetSVG = function (_) {
      if (!arguments.length) return planetSVG;
      planetSVG = d3.functor(_);
      return canvas_symbol;
    };
  ...
  function geo(cfg) {
    ...
    function go() {
      ...
      if (!isNaN(lon) && !isNaN(lat)) {
        ...
        zenith[2] = 0;
        let planet_ids = ['mer', 'ven', 'ter', 'mar', 'jup', 'sat', 'ura', 'nep', 'sol', 'lun']
        let srcs = [];
        for (let planet_id of planet_ids) {
          srcs.push(getPlanetSVG({ code: planet_id }))
        }
        let img_promises = [];
        for (let imgsrc of srcs) {
          let img_promise = loadImage(imgsrc);
          img_promises.push(img_promise)
        }
        function loadImage(src) {
          return new Promise(function (resolve) {
            let set_img = new Image();
            set_img.crossOrigin = 'anonymous';
            set_img.src = src;
            set_img.onload = () => resolve(set_img);
            set_img.onerror = () => resolve();
          });
        }
        Promise
          .all(planet_ids.map(id=>{
            return loadImage(getPlanetSVG({ code: id }))
              .then(loadedImg=>{
                return {
                  id: id,
                  img: loadedImg,
                  url: getPlanetSVG({ code: id })
                }
              })
          }))
          .then(function (planet_res) {
            //console.log('ALLIMAGESLOADED',planet_res);
            planet_objects = planet_res;
  ...
  function exportSVG(fname) {
    ...
    if (cfg.constellations.bounds) {
      q.defer(function (callback) {
        ...
        var conb = getData(constellations_borders_data, cfg.transform);
        if (Celestial.constellation) {
          var re = new RegExp("\\b" + Celestial.constellation + "\\b");
        }
    ...
    if (cfg.constellations.names) {
      q.defer(function (callback) {
        ...
        var conn = getData(constellations_data, cfg.transform);
    ...
    if (cfg.stars.show) {
      q.defer(function (callback) {
        ...
        var cons = getData(cfg.stars.data === 'stars.6.json' ? star6_data : star8_data, cfg.transform);
    ...
    function clip(coords, adj = 1) {
      return proj.clip && d3.geo.distance(center, coords) > halfπ * adj ? 0 : 1;
    }
